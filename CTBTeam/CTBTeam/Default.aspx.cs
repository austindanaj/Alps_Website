		...
		protected void download(object sender, EventArgs e) {
			...
			Lambda addColumns = new Lambda(delegate (object o) {
				...
				for (i = 0; i < d.Rows.Count; i++) {       //Forall rows in projectData: 
					...
					projectDataTable.Columns.Add(s, typeof(int));  //Add it as a column to the temporary datatable. The column accepts integer values because we're talking about hours worked
				}
			});
			...
			DataRow[] tempTable = new DataRow[partTimeEmployees.Rows.Count];
			...
			File.Create(@"" + Server.MapPath("~/Logs/DBLog.txt"));
			StreamWriter file = new StreamWriter(@"" + Server.MapPath("~/Logs/DBLog.txt"));
